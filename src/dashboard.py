...
try:
        ...
        with col3:
            ...
            try:
                ...
                if income is not None and not income.empty:
                    ...
                    if available_rows:
                        top_line = income.loc[available_rows].iloc[:, :2]
                    else:
    ...
    try:
        ...
        if news and len(news) > 0:
            ...
            for item in news[:3]:
                # yfinance now nests news details inside a 'content' dictionary
                content = item.get('content', item) 
                
                title = content.get('title', 'No title')
                
                # The publisher might be nested under 'provider' now
                provider = content.get('provider', {})
                publisher = provider.get('displayName', 'Unknown source') if isinstance(provider, dict) else 'Unknown source'
                
                # The link is now often under 'clickThroughUrl' or 'canonicalUrl'
                click_through = content.get('clickThroughUrl', {})
                link = click_through.get('url', content.get('canonicalUrl', '#')) if isinstance(click_through, dict) else '#'
